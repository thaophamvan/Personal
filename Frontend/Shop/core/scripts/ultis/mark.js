...
(function(global, factory) {
  ...
  class DOMIterator {
    ...
    static matches(element, selector) {
      ...
      const fn = element.matches || element.matchesSelector || element.msMatchesSelector || element.mozMatchesSelector || element.oMatchesSelector || element.webkitMatchesSelector;
      if (fn) {
        let match = false;
        selectors.every((sel) => {
    ...
    getContexts() {
      ...
      ctx.forEach((ctx) => {
        const isDescendant = filteredCtx.filter((contexts) => contexts.contains(ctx)).length > 0;
    ...
    isIframeBlank(ifr) {
      ...
      const { href } = ifr.contentWindow.location;
    ...
    iterateThroughNodes(whatToShow, ctx, eachCb, filterCb, doneCb) {
      ...
      while (retrieveNodes()) {
        if (this.iframes) {
          this.forEachIframe(
            ctx,
            (currIfr) => this.checkIframeFilter(node, prevNode, currIfr, ifr),
            (con) => {
  ...
  class RegExpCreator {
    constructor(options) {
      this.opt = { diacritics: true, synonyms: {}, accuracy: 'partially', caseSensitive: false, ignoreJoiners: false, ignorePunctuation: [], wildcards: 'disabled', ...options };
    }
    ...
    sortByLength(arry) {
      return arry.sort((a, b) => (a.length === b.length ? (a > b ? 1 : -1) : b.length - a.length));
    }
    ...
    createSynonymsRegExp(str) {
      ...
      const joinerPlaceholder = this.opt.ignoreJoiners || this.opt.ignorePunctuation.length ? '\u0000' : '';
      for (const index in syn) {
        if (syn.hasOwnProperty(index)) {
          ...
          keys = this.sortByLength(keys)
            ...
            .filter((k) => k !== '');
          if (keys.length > 1) {
            str = str.replace(new RegExp(`(${keys.map((k) => this.escapeStr(k)).join('|')})`, `gm${sens}`), `${joinerPlaceholder}(${keys.map((k) => this.processSynonyms(k)).join('|')})${joinerPlaceholder}`);
          }
        }
      }
      return str;
    ...
    createJoinersRegExp(str) {
      const joiner = [];
      const { ignorePunctuation } = this.opt;
  ...
  class Mark {
    ...
    set opt(val) {
      this._opt = { element: '', className: '', exclude: [], iframes: false, iframesTimeout: 5000, separateWordSearch: true, acrossElements: false, ignoreGroups: 0, each: () => {}, noMatch: () => {}, filter: () => true, done: () => {}, debug: false, log: window.console, ...val };
    }
    ...
    separateGroups(node, match, matchIdx, filterCb, eachCb) {
      ...
      for (let i = 1; i < matchLen; i++) {
        const pos = node.textContent.indexOf(match[i]);
    ...
    mark(sv, opt) {
      ...
      const handler = (kw) => {
        ...
        this[fn](
          regex,
          1,
          (term, node) => this.opt.filter(node, kw, totalMatches, matches),
          (element) => {
            matches++;
            totalMatches++;
            this.opt.each(element);
